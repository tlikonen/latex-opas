...
ja sen tarjoamaa ympäristöä.\avctan{pdflscape}
...
\item pääasiallinen perhe, joka on kirja\-typo\-gra\-fias\-sa usein
  antiikva eli pääteviivallinen (\englanti{serif, roman})%
...
\item tasalevyinen kirjoituskoneen kaltainen perhe
  (\englanti{type\-writer, mono\-spaced}).
...
\leijutlk{
  \begin{tabular}{llll}
    ...
    & {\rmfamily perhe} & perus, yl. antiikva, serif, roman \\
...
mutta joissakin fonteissa sama tehdään asetuksella
\koodi{Liga\-tures=\katk Dis\-cre\-tionary}. Useitakin ligatuurityyppejä
voi kytkeä päälle samanaikaisesti, ja seuraava esimerkki näyttää, miten
se tehdään:
...
tutkia \paketti{fontspec}\-/paketin ohjekirjasta.\avctan{fontspec}
Fontin ominaisuudet puolestaan selviävät käyttöjärjestelmän
komentotulkissa komennolla \koodi{otf\-info}. Komennon argumentiksi
annetaan muun muassa fonttitiedoston nimi.

\subsection{Muita fonttien asetuksia KESKEN}

% Oletus: Ligatures={TeX, Common} paitsi tasalevyisessä
% HyphenChar=- None (eri oletus tasalevyisessä, Xelatex, Lualatex)
% FakeStretch=
% OpticalSize=
% WordSpace=
% LetterSpace=
...
myötä, \paketti{babel} ei ihan pysynyt kehityksessä mukana. Syntyi
...
\subsection{Polyglossia KESKEN}

Kielipaketti \paketti{polyglossia} on tyypillinen valinta, jos halutaan
varmistaa, että Unicode\-/merkistöä voi käyttää vapaasti. Samalla se
tarkoittaa, että kääntäjän on oltava Lualatex tai Xelatex. Kielipaketin
käyttöönotto dokumentin esittelyosassa näyttää seuraavanlaiselta:
...
\koodi{\keno text\-english} tai \koodi{\keno text\-german}. Toinen
vaihtoehto on käyttää kielen nimen mukaista ympäristöä.
...
Tällä tavoin kielen mukaan merkitty teksti voi näyttää ladotussa
dokumentissa ihan samalta kuin muukin teksti. Tavutuksessa kuitenkin
käytetään kyseisen kielen asetuksia ja myös oletusfontti voi olla eri.
Muiden kuin pääasiallisen kielen teksti ladotaan oletuksena
pääasiallisella fontilla \koodi{\keno rm\-fam\-i\-ly} (luku
\ref{luku:kirjaintyypit}), mutta fonttiin voi vaikuttaa määrittelemällä
komennon, joka alkaa kielen nimellä ja päättyy sanaan \koodi{font},
esimerkiksi \koodi{\keno english\-font}.

\begin{koodilohkosis}
  \newcommand{\englishfont}{\itshape} % kursiivi
  \newcommand{\englishfont}{}         % sama fontti kuin ympärillä
  \newfontfamily{\englishfont}{…}[…]  % eri kirjainperhe
\end{koodilohkosis}

Edellisessä on kolme erilaista esimerkkiä englanninkielisen tekstin
fontti

komento määriteltiin tyhjäksi, eli se ei tee
mitään. Tällainen määrittely saattaa olla tarpeen silloin, kun haluaa,
että kielen teksti ladotaan samalla fontilla kuin muukin ympärillä oleva
teksti.


% { \sffamily tekstiä \textenglish{english} tekstiä }
...
\footnote{Teknisesti 8\=/bittiset merkistöt sisältävät
  2\textsuperscript{8} eli 256 merkkipaikkaa, mutta osa merkeistä on
  teknisiä ohjauskoodeja, jotka eivät liity ihmiskieliin.} %
ja sopivat tietyn rajallisen kielijoukon kirjoittamiseen. Nykyään
\paketti{babel} toimii myös Unicode\-/merkistön kanssa ainakin
latinalaiseen kirjaimistoon perustuvien kielten osalta.
