...
on tarkoitettu laajoihin dokumentteihin, ja niissä on
kirja\-typo\-grafian piirteitä.
...
(\komento{section}, \komento{subsection} jne.). Tekstin jäsentämistä ja
...
on tarkoitettu tekstin tiivis\-telmä\-osan tekemiseen; \luokka{book}\-/
...
Muita perusdokumenttiluokkia ovat \luokkam{let\-ter} ja \luokka{slides}.
Nimensä mukaisesti \luokka{let\-ter} on kirjeiden kirjoittamista varten.
...
samanmuotoisia kirjeitä usealle vas\-taan\-otta\-jalle. Sama
...
esi\-tys\-gra\-fiikka\-ohjel\-milla, joilla tehdään dia\-esi\-tyk\-siä
esimerkiksi esitelmien havaintomateriaaliksi. \luokka{slides}\-/ luokkaa
ei varmaan tarvita enää mihinkään, mutta esi\-tys\-gra\-fiik\-kaan on
olemassa erittäin monipuolinen luokka \luokka{beamer}, jonka
perus\-asioita käsitellään luvussa \ref{luku:diaesitykset}.
...
Sivu on oletuksena pysty\-asen\-nossa, mutta valitsin \koodim{landscape}
...
\ref{luku:kappale}). Eri makropaketit voivat hyödyntää \koodi{draft}\-/
valitsinta omalla tavallaan eli muuttaa toimintaansa sen perusteella.
Esimerkiksi grafiikkaan liittyvä paketti \paketti{graph\-icx} (luku
...
\paketti{hyper\-ref}\-/paketti jättää pdf:n sisäiset ristiviitteet ja
...
Matemaattisten kaavojen ladontaan liittyvässä \ymparisto{equa\-tion}\-/
...
sivulleen (\koodi{title\-page}) vaiko ensimmäisen sisältösivun alkuun
...
palstaa, mutta \paketti{multicol}\-/makropaketin avulla saa enemmänkin.
Palstoja käsitellään luvussa \ref{luku:palstat}.

Dokumenttiluokassa \luokka{slides} toimii \koodim{clock}\-/ valitsin,
joka latoo kellon\-ajan piirtoheitinkalvon loppuun \komento{note}\-/
...
mutta hieman helppokäyttöisempi luokka on \luokka{pow\-er\-dot}. Nämä
...
käsi\-ohjel\-mien tekemiseen soveltuu \luokka{leaf\-let}\-/luokka.
\textsc{cd}\-/levyjen kansia voi tehdä \luokka{cd}\-/luokan avulla.
Kuvitettuja seinäkalentereita vartenkin on tehty oma luokkansa,
\luokka{wall\-cal\-en\-dar}. Kitaran tablatuureja eli otelautakuvaan
perustuvia nuotteja voi tehdä \luokka{gui\-tar\-tabs}\-/luokan avulla.
...
Sivu on oletuksena pysty\-asen\-nossa (\koodim{por\-trait}), mutta sen
voi asettaa vaaka\-/asentoon valitsimella \koodim{land\-scape}. Tämä
asetus on voimassa koko dokumentin ajan. Jos haluaa asettaa vain
...
\koodim{margin\-par\-width}, ja palstan etäisyys sivun varsinaisesta
tekstialueesta määritellään valitsimella \koodim{margin\-par\-sep}.
Kumpikin valitsin tarvitsee argumentiksi mitan. Jos haluaa vaihtaa
huomautukset sivun vastakkaiseen marginaaliin, lisätään mukaan valitsin
...
\koodim{twocolumn} asetetaan, kumpi tila on oletuksena päällä. Kesken
dokumentin voi vaihtaa yksipalstaiseen tilaan komennolla
\komento{onecolumn} tai kaksipalstaiseen tilaan komennolla
\komento{twocolumn}. Käyttämällä \paketti{multicol}\-/
pakettia\avctan{multicol} voi olla useampiakin kuin kaksi palstaa.
Palstoja käsitellään tarkemmin luvussa \ref{luku:palstat}.

Valitsimen \koodim{column\-sep} avulla asetetaan palstojen välinen
etäisyys. Tämä valitsin on asettaa mitan \mitta{columnsep}, jota voi
muokata myös komennolla \komento{setlength}, kuten muitakin Latexin
mittoja (luku \ref{luku:mitat}).
...
\paketti{fancyhdr}\avctan{fancyhdr} toimintojen avulla. Silloin
sivutyylinä voi olla myös \koodim{fancy}:
...
Makropaketin \paketti{fancyhdr} avulla voi myös määritellä Latexin
...
uudelleen komennot \komento{headrulewidth} ja
\komento{footrulewidth}. Komennon määritelmäksi kirjoitetaan Latexin
mitta. Seuraavassa esimerkissä asetetaan yhden typografisen pisteen
(1\,bp) levyiset viivat. Mitan voisi asettaa myös nollaksi (0\,bp),
jolloin erotinviiva katoaa kokonaan näkyvistä.
...
\komento{markright}. Esimerkkiin \ref{esim:fancyhdr_koko} on koottu
...
\komento{usepackage}. Fonttikohtaisia makropaketteja on olemassa
...
(\komento{setsansfont}, \komento{setmonofont}) käytetään
...
kirjainperheen skaalausta hienosäätää vielä \koodi{Scale\-Again}\-/
...
etäisyys. Fontin koko ja rivikorkeus määritellään saman\-aikaisesti,
koska ne ovat saman \komento{fontsize}\-/ komennon argumentteja.
Esimerkki:
...
tulee komennon jälkeen. Vaikutus\-alue rajoittuu nykyisen ympäristön
(luku \ref{luku:ymparistot}) sisään tai aaltosulkeilla (luku
...
\providecommand{\rivi}{}
\renewcommand{\rivi}[4]{%
  \komento{#1} & \komento{#2}\koodi{\{\ldots\}} & #3 & #4 \\}
...
leikkaus, joka hieman mukailee käsi\-ala\-kir\-joi\-tusta, joskaan
kirjaimia ei ole sidottu toisiinsa. Sen sijaan kalteva leikkaus on
tavallisen eli pys\-ty\-asen\-toi\-sen leikkauksen kallistettu versio.
Kallistus voi olla kirjainmuotoilijan piirtämä leikkaus, tai kallistus
on voitu toteuttaa mekaanisesti. Kallistuksen voi tehdä myös Latexissa
(luku \ref{luku:fontit_venytys}).
...
Fontti\-asetusten valitsimella \koodim{Letter\-Space} säädetään merkkien
...
\label{luku:fontit_keinopienversaali}
...
tapa tavutusvihjeiden kirjoittamiseen on \komento{hyphenation}\-/
...
laatikkoon, joka pitää merkit yhdessä: \komento{mbox}\koodi{\{-aika\}}.
...
lyhemmät muodot, jos paketin lataa käyttämällä \koodi{short\-cuts}\-/
valitsinta.
...
\komento{discretionary}\-/komentoa (luku \ref{luku:tavutus_muut}) voi
...
kielipaketin komennolla \komento{disablehyphenation}.
\komentomargin{disable\-hyphenation} Tavutuksen saa takaisin päälle
komennolla \komento{enablehyphenation}.
...
\komento{texttt}\-/ komennon argumenttina olevaa tekstiä ei tavuteta.
Fontteja ja niiden asetuksia käsitellään luvussa
