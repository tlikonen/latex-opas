...
\subsection{Fonttien oletus\-asetukset}

Oletuksena%
\koodimargin{\keno setmainfont \\ \keno setsansfont} Latex\-/dokumentin
peruskirjainperheessä (\koodi{\keno set\-main\-font}) ja
pääteviivattomassa kirjainperheessä (\koodi{\keno set\-sans\-font}) ja
on päällä seuraavat asetukset:
...
\ref{luku:yhdys_ajatus_miinus}. \koodi{Common}\-/ligatuurit ovat
yleisimpiä typografisia ligatuureja kuten fi, ff ja fl, joita
käsitellään luvussa \ref{luku:typo_liga}. Molemmat saa pois päältä
...
kanssa. Sen sijaan kaikilla kääntäjillä tavutuksen voi kytkeä
...
tarkoitetut Tex\-/ligatuurit päällä. Oletusasetukset ovat seuraavat:
...
samanlevyisiä eikä yhden merkin tilaan sovi sulloa useampaa. Kaikkiin
tasalevyisiin fontteihin ei ligatuurimerkkejä ole edes toteutettu.

Oletuksena tavutus on kytketty pois päältä, koska tasalevyistä fonttia
käytetään tavallisesti tietokoneisiin liittyvien koodien latomiseen,
eikä niitä haluta yleensä tavuttaa. Tavutuksen saa kuitenkin päälle
antamalla arvoksi yhdysmerkin (\koodi{Hyphen\-Char=\katk \{\=/\}}) tai
Xelatex\-/kääntäjällä minkä tahansa merkin.

Komennolla%
\koodimargin{\keno default\-font\-features} \koodi{\keno
  default\-font\-features} voi asettaa (joidenkin) kirjainperheiden
oletusasetukset, kun komennon suorittaa ennen muiden kirjainperheiden
määrittelyä.
...
Komennon valinnaisella argumentilla voi rajata, mitä kirjainperhettä
...
kytketään päälle: \koodi{\keno rm\-family}, \koodi{\keno sf\-family},
\koodi{\keno tt\-family} (taulukko \ref{tlk:fonttimallikomennot},
s.~\pageref{tlk:fonttimallikomennot}) tai komennolla \koodi{\keno
  new\-font\-family} määritelty fonttikomento.
...
\begin{koodilohkosis}
  \setmainfont{…}[Ligatures={TeX, Common, Historic, Discretionary}]
\end{koodilohkosis}
...
\subsection{Muita asetuksia KESKEN}

Harvemmin tarvittavia mutta välillä hyödyllisiä fonttien asetuksia ovat 

sana \\
{\addfontfeatures{LetterSpace=10} \scshape harvennus}
...
pois oletuskielestä suoritetaan \koodi{\keno rm\-fam\-i\-ly}\-/komento
...
ajaksi tulee käyttöön perusfontti \koodi{\keno rm\-fam\-i\-ly} eikä
ympärillä oleva pääteviivaton \koodi{\keno sf\-fam\-i\-ly}.
...
kirjainperheeseen tai \=/leikkaukseen vaikuttavia komentoja, joita on
koottu taulukkoon \ref{tlk:fonttimallikomennot}
(s.~\pageref{tlk:fonttimallikomennot}). Komentojen määrittelemistä
käsitellään luvussa \ref{luku:komennot-määr}.
...
Eri%
\koodimargin{\keno babel\-font} kielille on mahdollista asettaa eri
kirjainperheet \koodi{\keno babel\-font}\-/komennolla. Tämä komento
korvaa \paketti{fontspec}\-/paketin fontinvalintakomennot (luku
