...
kirjakkeen päähän valettua kirjaimen tai muun merkin kohokuviota, joka
...
(\textenglish{sans serif, gothic}) ja tasalevyinen kirjoituskoneen
kaltainen perhe (\textenglish{typewriter, monospace}). Kuvassa
\ref{kuva:kirjainperhetyypit} ovat tässä oppaassa käytetyt kolme eri
kirjainperhettä. Leipätekstissä käytetään antiikvaa (Libertinus Serif),
otsikoissa ja kuvateksteissä groteskia (Libertinus Sans%
\footnote{Tämän oppaan groteskifontti Libertinus Sans on tyyliltään
  lähellä antiikvaa -- ainoastaan pääteviivat puuttuvat. Tämäntyyppisiä
  fontteja sanotaan joskus humanistisiksi groteskeiksi tai
  pääteviivattomiksi antiikvoiksi.}) ja koodi\-esi\-mer\-keis\-sä
tasalevyistä (Libertinus Mono). Kirjoituskoneen kaltainen tasalevyinen
kirjainperhe on tässä tapauksessa tyypiltään antiikva eli
pääteviivallinen, mutta se voisi olla muutakin. Tasalevyisyys on sen
kirjainperheen tärkein määrittävä tekijä Latexin asetusten näkökulmasta.
...
\leijukuva{
  ...
  \caption{Vasemmalla pääteviivallinen (Libertinus Serif), keskellä
    pääteviivaton (Libertinus Sans) ja oikealla tasalevyinen
    pääteviivallinen kirjainperhe (Libertinus Mono)}
...
fontti\-asetuksille ja joillekin kirjainperhekokonaisuuksille.
Esimerkiksi tämän oppaan Open Type \=/kirjainperheet (Libertinus) on
ladattu alla olevalla komennolla. Valitsin \koodi{old\-style} tarkoittaa
gemenanumeroita eli pien\-aak\-kos\-nu\-me\-roi\-ta (1\,6\,9).
...
otetaan käyttöön \paketti{fontspec}\-/paketin komennoilla seuraavan
...
kuten ligatuureja, optisia kokoja ja muita fontin ominaisuuksia.
Edellisessä esimerkissä käytetään vain \koodi{Scale}\-/valitsinta, jolla
fontin voi skaalata haluttuun kokoon.

Peruskirjainperheen (\koodi{\keno setmainfont}) skaalaukseksi
asetetaan~1, eli sille ei tehdä mitään, ja koko valitsimen voisi jättää
pois. Sen sijaan kahdella muulla kirjainperheellä (\koodi{\keno
  setsansfont}, \koodi{\keno setmonofont}) käytetään
...
pituus on \murtoluku{1/72} tuumaa eli 0,3528 millimetriä.
Kirjainleikkauksen pistekoko mitataan merkistön ylimmän ja alimman
kohdan välillä, ja siihen luetaan mukaan ylä- ja alapuolella oleva pieni
tyhjä tila, jonka fontin suunnittelija on määritellyt.
...
määritellään saman\-aikaisesti, koska ne ovat saman \koodi{\keno
  fontsize}\-/komennon argumentteja. Esimerkki:
...
kirjainperheen ja kirjainleikkauksen valintaan. Perhe- ja
leik\-kaus\-ase\-tuk\-set ovat hierarkkisia, eli kirjainleikkauksen
muuttaminen valitsee toisen leikkauksen samasta perheestä.
...
'muoto'. Kummastakin ryhmästä on valittuna aina yksi, eli
samanaikaisesti voi olla voimassa esimerkiksi \koodi{\keno bfseries}
(lihavoitu) ja \koodi{\keno itshape} (kursiivi), ja tuloksena on
\textbf{\itshape lihavoitua kursiivia}.

Tämän oppaan fontissa kalteva leikkaus (\koodi{\keno slshape}) ja
kursiivileikkaus (\koodi{\keno itshape}) tuottavat saman lopputuloksen.
...
puolestaan on aina muodoltaan erilainen leikkaus, ja se hieman
jäljittelee yhtenäistä käsi\-ala\-kir\-joi\-tusta.
...
\begin{koodilohkosis}
  \begin{LARGE}
    \begin{scshape}
      Tämä teksti on isoa (LARGE) pienversaalia (scshape).
...
vähintäänkin määritellä komento \koodi{\keno normalsize} mutta sen
