...
tekijöiden nimien ja päiväyksen latomiseen (luku \ref{luku:kansisivu}),
...
helppokäyttöisempi luokka on \luokka{pow\-er\-dot}. Nämä luokat ovat
eräänlaisia Latexin vastineita esimerkiksi \englanti{Libreoffice
  Impress}\-/{} ja \englanti{Microsoft Powerpoint} \=/ohjelmille.
...
sivuja vaaka\-/asentoon, täytyy käyttää \paketti{pdflscape}\-/
pakettia\avctan{pdflscape} ja sen tarjoamaa ympäristöä.
...
Valitsimen \koodim{columnsep} avulla asetetaan palstojen välinen
...
mutta perusteellisemmin niitä käsitellään \paketti{geometry}\-/
paketin\avctan{geometry} ohjekirjassa.

Taulukossa \ref{tlk:sivun_mittoja} mainittu mitta \mitta{linewidth}
eroaa tekstialueen leveysmitasta \mitta{textwidth} esimerkiksi silloin,
...
\section{Pdf-tiedosto KESKEN}
\label{luku:pdf_asetukset}

Pdf-tiedostot voivat sisältää metatietoja kuten dokumentin nimen,
tekijän ja päiväyksen. Niille voi määrittää myös monenlaisia asetuksia
kuten ristiviitteiden ja linkkien ulkoasun tai katselutilan, jossa
dokumenttia ehdotetaan katsottavan. Pdf\-/ tiedostot sisältävät oman
sisäisen sisällysluettelonsa, jonka ominaisuuksiin voi vaikuttaa
Latexista käsin.

Pdf-tiedoston asetukset toteutetaan \pakettim{hyperref}\-/
paketin\avctan{hyperref} avulla. Paketti neuvotaan lataamaan muiden
pakettien jälkeen, koska se lisää ominaisuuksia muihin komentoihin.
Paketin voisi ladata esimerkiksi seuraavalla tavalla:
...
ja niiden arvoja. Monilla valitsimilla voi vaikuttaa pdf\-/ tiedoston
asetuksiin kuten metatietoihin ja katselutilaan.
...
\begin{koodilohko}
  \hypersetup{
    ...
    pdfinfo={
      ...
      Subject={Latex-ladontajärjestelmän opas},
...
sen, että pdf:ssä olevia linkkejä ei merkitä millään tavalla. Oletuksena
linkit kehystetään eri väreillä riippuen linkin tyypistä. Valitsin
\koodi{bookmarksopen} näyttää pdf:n sisällysluettelon kokonaan avattuna.
Oletuksena alaluvut on piilotettuna, ja ne joutuu avaamaan
napsauttamalla hiirellä ylemmäntasoisen otsikon avauspainiketta.
Valitsin \koodi{bookmarksnumbered} näyttää pdf:n sisällysluettelossa
lukujen numeroinnin (kuten 1.1, 1.2, 2.1 tms.). Valitsimella
\koodi{pdfinfo} määritellään pdf:n metatietoja kuten otsikko, aihe ja
tekijä mutta omiakin metatietoja voi lisätä.

% Vielä: katselutila yms.: pdfpagelayout, pdfview, pdfstartview,
% pdfremotestarview.
...
(\komento{setsansfont}, \komento{setmonofont}) käytetään kerroinasetusta
...
Mikäli skaalausasetus \koodi{MatchLowercase} ei tuota ihan toivottua
