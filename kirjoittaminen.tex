...
\mitta{emergencystretch} riittävän suureksi. \komentomargin{fussy}
Sanavälien venymiseen ja kappaleiden tasaiseen latomiseen liittyvät
asetukset voi palauttaa oletusarvoihin komennolla \komento{fussy}.
...
\mittam{par\-skip} avulla. Se on oletuksena nolla, mutta pientä
venymistä kuitenkin sallitaan, eli joissakin tilanteissa kappaleiden
väliin voidaan latoa pieni tyhjä tila. Jos tyhjää tilaa ei haluta
missään tilanteessa, asetetaan mitta vain nollaksi:
...
Paketti \paketti{no\-widow} määrittelee myös komennot \koodi{\keno
...
vaikkapa nimellä \koodi{\keno huomautus}, jota käyttämällä
...
vaikuttava komento \koodi{\keno Ragged\-Right}, josta on lisätietoa
luvussa \ref{luku:kappaleen_tasaus}.
...
kuten pää- ja ala\-otsi\-kon, erillisen johdantokappaleen eli ingressin,
...
komennon. Jokainen anfangi on erilainen, ja siksi \koodi{\keno
  lettrine}\-/ komentokin sisältää useita asetuksia anfangin ulkoasun
...
ohjekirjasta tietoa \koodi{\keno lettrine}\-/ komennon valitsimista ja
myös teknisistä rajoituksista. Anfangit eivät toimi Latexissa teknisesti
joka paikassa -- ja typografisesti vielä harvemmassa.
...
\ref{luku:fontit_korkea}. \marginaali{Katso fontti\-komentoja taulukosta
...
parhaiten saman perheen \textit{kursiivi} (\koodi{\keno text\-it}).
\koodimargin{\keno textit} Lihavointi toimii huonommin antiikvan kanssa,
koska pelkkä merkkien viivojen vahvistaminen ei tuo riittävää
kontrastia.
...
\textbf{Lihavointi} (\koodi{\keno text\-bf}) \koodimargin{\keno textbf}
toimii huomattavasti paremmin groteskityyppisen kirjainperheen kanssa,
koska lihavoitu leikkaus on usein selvästi vahvempi normaaliin
verrattuna. Groteskin kursiivi sen sijaan ei monesti edes ole
varsinainen kursiivileikkaus vaan pelkkä lievä kallistus
\koodimargin{\keno textsl} (\koodi{\keno text\-sl}). Sellainen ei
välttämättä tuo riittävää muotokontrastia.\footnote{Joissakin niin
  sanotuissa humanistisissa groteskiperheissä on mukana ihan
  käyttökelpoinen ja selvästi erottuva kursiivileikkaus.}
...
Latexissa kursiivin voi toteuttaa myös komennolla \koodi{\keno emph},
\koodimargin{\keno emph} joka huomioi ympärillä olevan korostustilan.
Tavallisesti komento kursivoi argumentiksi annetun tekstin, mutta jos
kursivointi on jo päällä komennon ympärillä, se latookin argumentiksi
annetun tekstin tavallisella pystyasentoisella kirjainleikkauksella.
Komennon ajatuksena on, että voidaan samalla komennolla korostaa sekä
pystyasentoisen tekstin että kursiivin keskellä, eikä tarvitse välittää,
...
Saattaa olla hyödyllistä käyttää \koodi{\keno emph}\-/ komentoa
esimerkiksi lainatuissa tekstipätkissä. Lainattu teksti voidaan
tilanteen mukaan latoa tavallisella tai kursiivileikkauksella, ja
korostukset silti toimivat.
...
kapiteeli, \koodimargin{\keno textsc} jonka kirjaimet näyttävät
...
merkkien koko\-ero on häiritsevän suuri. Kuten seuraavista esimerkeistä
...
Latexin oma alleviivauskomento \koodi{\keno under\-line}
\koodimargin{\keno under\-line} piirtää viivan tekstin tai muun
argumentiksi annetun sisällön alapuolelle. Komennon huono puoli on, että
erilliset alleviivaukset eivät välttämättä osu samalle korkeudelle.
Seuraavassa esimerkissä toisen sanan alleviivaus on alempana j- ja
y\=/kirjainten alapidennyksen vuoksi.
...
Toinen \koodi{\keno under\-line}\-/komennon ongelma on, että se estää
sanojen tavuttamisen ja muutenkin tekstin katkaisemisen rivin lopussa.
Komennon argumentiksi annettua tekstiä ei katkaista edes sanavälien
kohdalta. Komennon hyvä puoli on, että se toimii myös
matematiikkatilassa (luku~\ref{luku:matematiikka}).
...
sisältää kak\-sois\-alle\-vii\-vauk\-sen, aaltoviivat, pisteviivat ja
erilaisia ylikirjoitusmerkintöjä. Taulukkoon \ref{tlk:ulem} on koottu
paketin sisältämiä komentoja.

\providecommand{\rivi}{}
\renewcommand{\rivi}[2]{\koodi{\keno #1} & #2}
...
\leijutlk{
  ...
  \caption{Alleviivaus- ja ylikirjoitusmerkintöjä
    \paketti{ulem}\-/paketin komentojen avulla}
  \label{tlk:ulem}
}

Oletuksena \paketti{ulem}\-/ paketti määrittelee Latexin \koodi{\keno
  emph}\-/ korostuskomennon (luku \ref{luku:peruskorostukset}) uudelleen
siten, että komento tuottaa kursiivin sijasta alleviivattua tekstiä.
Ajatuksena lienee se, että paketin avulla voi helposti muuttaa
...
Paketin \paketti{ulem} myötä on käytettävissä mitta \koodi{\keno
  UL\-depth}, joka on alleviivauksen etäisyys tekstin peruslinjasta.
Mitta asetetaan tavalliseen tapaan \koodi{\keno set\-length}\-/
komennolla (luku \ref{luku:mitat}). Viivan paksuus puolestaan sisältyy
komennon \koodi{\keno UL\-thick\-ness} määritelmään, ja komennon voi
halutessaan määritellä uudestaan. Seuraavassa on näistä esimerkki:
...
\koodi{\keno text\-tt}. \koodimargin{\keno texttt} Tasalevyisellä
fontilla ladottava teksti on tavallisesti tietokonekoodia, joten sille
on asetettu hieman toisenlaiset oletusasetukset kuin muille
kirjainperheille. Oletuksena tekstiä ei tavuteta, ja esimerkiksi
\koodi{\keno text\-tt}\-/komennon argumentiksi annettu teksti
katkaistaan vain sanavälien ja yhdysmerkkien kohdalta. Myös niin sanotut
Tex\-/ligatuurit (esim. \koodi{''} ja~\koodi{--}) on kytketty pois
päältä. Fonttien oletusasetuksia ja niiden muuttamista käsitellään
luvussa \ref{luku:fontit_oletusasetukset}. Tex\-/ligatuureja eli Texin
...
Toinen vaihtoehto tasalevyisen tekstin tuottamiseen on \koodi{\keno
  verb}\-/ komento. \koodimargin{\keno verb} Komennon nimi on lyhennetty
englannin kielen sanasta \textit{\englanti{verbatim}}, joka tarkoittaa
'kirjaimellisesti' tai 'sananmukaisesti'. Niin se juuri toimiikin, eli
\keno{verb}\-/ komennolle annettu argumentti tulkitaan kirjaimellisesti.
Texin omat merkintätavat eivätkä kenoviivalla (\koodi{\keno}) alkavat
komennot eivät toimi tämän komennon argumentissa.

\koodi{\keno verb}\-/ komennon argumentti täytyy ilmaista tavallisesta
...
\koodi{ver\-ba\-tim}\-/ ympäristö, \koodimargin{verbatim} jonka sisällä
kaikki teksti tulkitaan kirjaimellisesti -- myös välilyönnit ja
rivinvaihdot.
...
\paketti{fancy\-vrb}.\avctan{fancyvrb} Sen \koodi{Ver\-ba\-tim}\-/
ympäristölle \koodimargin{Verbatim} voi antaa monenlaisia asetuksia,
joilla vaikutetaan tietokonekoodin latomiseen. Koodiin saa mukaan
esimerkiksi rivinumerot, ja sen voi latoa kehyksen sisään. Myös koodin
...
makropaketin avulla. Paketti \paketti{hyper\-ref}\avctan{hyperref}
...
\paketti{hyper\-ref}\-/paketin ohjekirja neuvoo lataamaan paketin
...
Edellisen esimerkin komennolla \komento{hyper\-setup} vaikutetaan
...
Esimerkin seuraava komento \komento{url\-style} määrittää, mitä
...
\komento{url\-style}\-/ komennon argumentilla \koodi{rm} (antiikva,
...
\koodimargin{\keno href} jolle annetaan kaksi pakollista argumenttia:
ensimmäinen on varsinainen osoite ja toinen on dokumenttiin ladottava
teksti. Linkin teksti voi siis olla mitä hyvänsä tekstiä, ja pdf\-/
lukijassa tekstiä napsauttamalla linkin osoittama sivusto avautuu.
...
Jos halutaan latoa dokumenttiin verkko\-/osoite, joka ei ole
pdf\-/dokumentissa napsautettava linkki, käytetään komentoa
\komento{no\-link\-url}. \komentomargin{no\-link\-url} Tämän komennon
argumenttina oleva verkko\-/osoite ainoastaan ladotaan osoitteiden
tavoin.

Linkkinä toimivien sähköpostiosoitteiden latominen vaatii
\komento{href}- ja \komento{no\-link\-url}\-/komentojen yhdistämistä.
