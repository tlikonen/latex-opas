...
nimeltä \paketti{bib\-latex-ext}\avctan{biblatex-ext} laajentaa
...
yleensä \paketti{biblatex}\-/paketin omia asetuskomentoja kuten
\koodi{\keno Declare\-Field\-Format} ja \koodi{\keno
  Declare\-Delim\-Format}.
...
kursivoidaan julkaistun kokoelman nimi (\koodi{book\-title}) tai
aikakauslehden nimi (\koodi{jour\-nal\-title}). Näissä teostyypeissä
viitatun artikkelin nimi (\koodi{title}) ladotaan lainausmerkkeihin.
Käytäntö tuntuu järkevältä, sillä kursivoituna on aina julkaistu
kokonainen teoskappale eikä sen osa. Käytännössähän tiedonlähde
joudutaan hakemaan teoskappaleen nimen perusteella. Joku voi silti
haluta muuttaa näiden ulkoasua ja esimerkiksi kursivoida aina
viittauksen kohteena olevan artikkelin. Seuraavassa on esimerkkikomennot
edellä mainittujen lähdeluettelon kenttien muuttamiseen.
...
pois, vaikutetaan kaikkiin teostyyppeihin. Ensimmäinen pakollinen
...
kohdalle. Teostietokannasta tulevan kentän tekstisisältö on parametrissa
\koodi{\#1}.
...
\koodi{author\-year} ei ole omaa kenttää teoksen päiväyksen ulkoasun
muuttamiseen, mutta jos käyttää tyyliä \koodi{ext-author\-year}, sekin
puute korjaantuu, ja voi käyttää \koodi{bib\-label\-date}\-/kenttää
sulkeiden poistamiseen.
...
% Edellä mainituilla \koodi{\keno Declare\-Field\-Format}\-/komennoilla ei
% lähdemerkinnän  määritellä välimerkkejä
