...
varsinaista Unicoden miinusmerkkiä \uctunnus{u+2212 minus sign}, koska se
...
tarke voi liittyö muihinkin kirjaimiin. Kaikki merkit voi kirjoittaa
...
\leijutlk{
  \begin{tabular}[t]{lll}
    ...
    \rivi{r\{a\}}{\r{a}}{yläpuolinen ympyrä}
    \rivi{v\{a\}}{\v{a}}{hattu}
    ...
    \rivi{.a}{\.a}{yläpuolinen piste}
    \rivi{d\{a\}}{\d{a}}{alapuolinen piste}
    \rivi{k\{a\}}{\k{a}}{ogonek}
...
Komennot voivat ottaa vastaan parametreja eli lisätietoa, jota komento
käsittelee ja tarvitsee toimintaansa. Jotkin parametrit voivat olla
pakollisia ja jotkin valinnaisia. Pakolliset parametrit kirjoitetaan
aaltosulkeisiin \koodi{\{\ldots\}} ja valinaiset hakasulkeisiin
\koodi{[\ldots]}.

\begin{koodilohkosis}
  \komento
  \toinenkomento{para}{metreja}
  \toinenkomento[valinnainen]{para}{metreja}
\end{koodilohkosis}
...
esimerkissä sana \emph{Latex} ladotaan ehjänä, jos vain \koodi{\keno
  komento} itsessään ei tulosta mitään eli ei vaikuta tekstin
latomiseen.
...
Komennon nimen ja parametrien välissä voi olla sanavälejä, ja ne kaikki
syödään pois. Komennon ja sen parametrit voi siis kirjoittaa vaikka
seuraavalla tavalla.
...
ladotaan erilaisella kirjainleikkauksella, kapiteelilla. Jos fontti vain
tukee kapiteelia, sitä saadaan helposti komennolla \koodi{\keno
...
\footnote{Komento \koodi{\keno text\-en\-glish} on Polyglossia\-/paketin
  ominaisuus.} Lähdedokumentin kirjoittaminen on ikävää ja ulko\-asu
sekava, jos Unicode\-/tunnusten kohdalla on aina \koodi{\keno
  text\-en\-glish\{\keno text\-sc\{\ldots\}\}}.
...
määritelmää on helppoa muuttaa. Itse tekstikin pysyy selkeänä, kun
siellä on vain korkean tason komentoja, jotka ilmaisevat tarkoituksen
eivätkä teknistä toteutusta.

\subsection{Komentojen määrittäminen}

Komentojen määrittelyyn on kolme erilaista komentoa, ja niille kaikille
annetaan samanlaiset parametrit. Komennot ovat seuraavat:
...
Ensimmäinen pakollinen parametri on komennon nimi (\koodi{\keno nimi}).
Komento \koodi{\keno newcommand} määrittelee uuden komennon, ja se antaa
virheilmoituksen, jos komento on jo olemassa. Toinen komento
\koodi{\keno renewcommand} määrittelee olemassa olevan komennon
uudelleen eli antaa sille uuden määritelmän. Se antaa virheilmoituksen,
jos komentoa ei ollut olemassa. Kolmas komento \koodi{\keno
  providecommand} puolestaan määrittelee uuden komennon vain siinä
tapauksessa, että sellaista ei ollut ennen olemassa. Se ei anna
virheilmoitusta.

Komentojen toinen pakollinen parametri (\koodi{määritelmä}) sisältää
...
\begin{koodilohkosis}
  ...
  \komento      % Minua komennettiin!
\end{koodilohkosis}

Komentojen ensimmäinen valinnainen parametri on luku (\koodi{n}), joka
kertoo, kuinka monta parametria määriteltävä komento käsittelee.
Määritelmässä voi käyttää muuttujia \koodi{\#1}, \koodi{\#2},
\koodi{\#3} jne., ja ne korvautuvat komennon suoritusvaiheessa
ensimmäisellä, toisella, kolmannella jne. parametrilla.

\begin{koodilohkosis}
  \newcommand{\komento}[2]{Sanoit #1 ja #2!}

  \komento{hip}{hei}    % Sanoit hip ja hei!
\end{koodilohkosis}

Toinen valinnainen parametri -- jos se on mukana -- kertoo, että
määriteltävän komennon ensimmäinen parametri on valinnainen ja että tämä
on sen oletus\-arvo (\koodi{oletus}). Oletus\-arvoa käytetään silloin,
kun valinnaista parametria ei ole annettu.

\begin{koodilohkosis}
  \newcommand{\komento}[3][tyyppi]{Hei #1, sanoit #2 ja #3!}

  \komento{hip}{hei}         % Hei tyyppi, sanoit hip ja hei!
  \komento[Leslie]{hip}{hei} % Hei Leslie, sanoit hip ja hei!
\end{koodilohkosis}
