...
gravisaksentti (\`{}) tehdään komennolla \komento[\{\}]{`}.
...
Makropaketti \pakettim{csquotes}\avctan{csquotes} sisältää
lainausmerkkeihin liittyviä komentoja ja kielikohtaista logiikkaa.
Paketissa olevan \komento{enquote}\-/komennon avulla voi jättää
...
(\komento[\{--\}]{texttt}), joka oletuksena kytkee pois Latexin
...
\paketti{realscripts}\-/ paketin avulla.\avctan{realscripts}
...
Latexin alkuperäiset komennot. Seuraavassa esimerkissä vertaillaan
...
tekemiä. Mekaaninen ylä- ja alaindeksitoiminto ohentaa merkkejä turhan
paljon, eikä se ymmärrä poistaa gemenanumeroita (3\,6\,2\,4) käytöstä
vaan latoo ne sellaisenaan suunnilleen oikeaan paikkaan.
...
\providecommand{\rivi}{}
\renewcommand{\rivi}[4][]{{\erikoisfontti #3} & \komento[#1]{#2} & #4}
...
\leijutlk{
  \begin{tabular}{cll}
    ...
    \midrule
    \rivi[\koodi{`{}`}]{\textquotedblleft}{textquotedblleft}{vasen lainausmerkki}
    \rivi[\koodi{'{}'}]{\textquotedblright}{textquotedblright}{oikea lainausmerkki}
    \rivi{\textquotedbl}{textquotedbl}{yleislainausmerkki (\textsc{ascii})}
    \rivi[\komento{lq} \koodi{`}]{\textquoteleft}{textquoteleft}{vasen puolilainausmerkki}
    \rivi[\komento{rq} \koodi{'}]{\textquoteright}{textquoteright}{oikea
    puolilainausmerkki, heittomerkki}
    \rivi{\textquotesingle}{textquotesingle}{yleispuolilainausmerkki ja
    -heittomerkki (\textsc{ascii})}
    \rivi[\koodi{<{}<}]{\guillemotleft}{guillemotleft}{vasen kulmalainausmerkki}
    \rivi[\koodi{>{}>}]{\guillemotright}{guillemotright}{oikea kulmalainausmerkki}
...
\leijutlk{
  \begin{tabular}{cll}
    ...
    \rivi[\koodi{?`}]{\textquestiondown}{textquestiondown}{ylösalainen kysymysmerkki}
    \rivi{\textgreater}{textgreater}{suurempi kuin -merkki}
    \rivi{\textless}{textless}{pienempi kuin -merkki}
    \rivi[\komento{ldots} \komento{dots}]{\ldots}{textellipsis}{kolme pistettä, ellipsi}
    ...
    \rivi[\komento{\^{}}\koodi{\{\}}]{\textasciicircum}{textasciicircum}{sirkumfleksi}
    \rivi[\komento{\~{}}\koodi{\{\}}]{\textasciitilde}{textasciitilde}{tilde}
...
Komento \komento{-} on tavutusvihje, joka neuvoo tavutusalgoritmia
...
käyttää taulukossa \ref{tlk:merkkien_suojaus}
...
nimen jälkeen aaltosulkeet (\komentox[\{\}]{komento}) tai kenoviiva ja
sanaväli (\komentox[\keno~]{komento}). Komennon nimen ja argumenttien
...
\begin{tulossis}
  Komennolla \komentox{section} tehdään otsikoita.
\end{tulossis}

Edellä mainitun komennon määritelmään voi olla tarpeen lisätä myöhemmin
muitakin asioita. Latexin komentojen nimet ovat englantia, ja jos nimen
tavuttaa, täytyisi se tehdä englannin sääntöjen mukaisesti. Esimerkkinä
olleen \komentox{komento}\-/ komennon määritelmässä voisi siis vaihtaa
myös kielen komennolla \komento{textenglish}.%
\footnote{Katso kieliasetuksia luvusta \ref{luku:kieliasetukset}.}
...
\footnote{Lisätietoa asiahakemistoista on luvussa \ref{luku:asiasanat}.}
Lopulta Latex\-/komentojen merkitsemiseen tarkoitettu \komentox{komento}
...
monimutkaisen teknisen toteutuksen. Myöhemmin toteutusta on helppoa
muuttaa, koska komennon määrittely on vain yhdessä paikassa
lähdedokumentin alussa.
...
tuovat tällaisen suojausominaisuuden.
...
komentomäärittelyn vaikutusaluetta. Seuraavan esimerkin alussa asetetaan
\komentox{komento} tiettyyn alkuperäismääritelmään. Aaltosulkeiden
sisällä se määritellään väliaikaisesti uudestaan. Aaltosulkeilla rajatun
ympäristön jälkeen komennon uusi määritelmä lakkaa ja komento palautuu
alkuperäiseksi.
...
\komento{ifthenelse}\-/ komennon avulla. Se on ohjelmointikielistä tuttu
ehtorakenne: jos annettu ehtolauseke on tosi, käsitellään then\-/haara;
muussa tapauksessa käsitellään else-haara.
...
lopettavan \komento{end}\-/ komennon jälkeiset sanavälit.
...
\komento{stretch}\-/ mittakomennon tai \komento{hfill}\-/ komentojen
avulla seuraavasti:
...
komennolla \komento{vfill}, joka tekee saman asian kuin \komento[\{0mm
plus 1fill\}]{vspace}.
...
esimerkiksi makropaketin koodissa: \koodi{fil} sallii, että käyttäjä tai
muu koodi syrjäyttää asetuksen; \koodi{filll} on ehdoton sääntö, joka
syrjäyttää muut.
...
nimeltä \laskurix{oma}. Laskurien nimet koostuvat pelkistä kirjaimista,
...
symbolit toimivat vain lukualueella 1--9.
...
\komento{thepage} tulostaa sivunumeron eli tekee käytännössä saman asian
kuin komento \komento[\{page\}]{arabic}. Aina nämä eivät
...
\komento{thetable} tulostaa ensin pääluvun ja sen perään pisteen ja
...
ristiviitteen numeron, joten tätä komentoa seuraava \komento{label}\-/
komento viittaa tähän laskuriin ja sen \komentox{thelaskuri}\-/ komennon
tulosteeseen. Ristiviitteitä käsitellään tarkemmin luvussa
\ref{luku:ristiviitteet}.

Makropaketti \pakettim{chngcntr}\avctan{chngcntr} helpottaa
...
laskurin arvon voi tulostaa. Se tulostaa mukaan myös ylemmäntasoisen
laskurin arvon. Jos tätä \komentox{the}\-/ alkuista komentoa ei halua
määritellä uudelleen, täytyy käyttää tähdellisiä komennon versioita:
...
\ref{luku:komennot-määr}), esimerkiksi seuraavalla tavalla:
...
viimeisen sivun numeron voi tulostaa komennoilla \komento{ref} ja
