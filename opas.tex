...
Toisella ja kolmannella rivillä käytetään komentoa \koodi{\keno
  usepackage} ja otetaan käyttöön fontti- eli kirjaintyyppi\-asetuksia
hoitava Fontspec\-/paketti ja kieli\-asetuksista vastaava
Polyglossia\-/paketti. Kumpaakin tarvitaan melkein joka kerta
dokumenteissa, ja niitä käsitellään tarkemmin luvuissa
\ref{luku:kirjaintyypit} ja \ref{luku:kieliasetukset}.
...
muitakin. Fontin oletuskoko on 10 pistettä,%
\footnote{Niin sanotussa pica\-/pistejärjestelmässä yksi piste on 1/72
  tuumaa. Fontin pistemitta mitataan kirjaimiston ylimmän ja alimman
  kohdan välillä.} mutta tässä esimerkissä se venytetään
1,3\-/kertaiseksi eli 13 pisteeseen. Riviväliin liittyvä kerroin
asetetaan rivillä 7.
...
Dokumentin alku\-osaa esimerkin riville 8 saakka kutsutaan
mää\-ri\-tel\-mä\-osak\-si (engl. \textenglish{\emph{preamble}}). Tässä
osassa ladataan tarvittavat makropaketit ja määritetään dokumentin
asetuksia ja taustatietoja. Riviltä 9 alkaa varsinainen dokumentti\-osa
eli sivulla näkyvä osa. Se osa kirjoitetaan
...
Latex\-/kirjailijan täytyy silloin tällöin lukea ohjekirjoja. Vaikka
itse Latexin joskus oppisikin ulkoa, ei voi koskaan muistaa kaikkien
makropakettien kaikkia ominaisuuksia. Myös uusia makropaketteja ja
ihmisten suosituksia tulee vastaan esimerkiksi verkkokeskusteluissa.

Tex Live \=/jakelun (ks. luku \ref{luku:asentaminen}) mukana tulee
mainio komentotulkissa toimiva ohjelma \koodi{texdoc}, jolla voi hakea
...
Todennäköisesti tulet käyttämään \koodi{texdoc}\-/komentoa usein.

\subsection{Latexmk}

Hyödyllinen komento on myös \koodi{latexmk}, joka helpottaa dokumenttien
...
tarpeen, mutta \koodi{latexmk}\-/ohjelma käynnistää uusintakäännöksen
ihan itse, aina kun se on tarpeellista.
...
aikana luodut väli\-aikaistiedostot (log, aux, out ym.), ja neljännen
rivin komento poistaa kaikki luodut tiedostot eli
väli\-aikais\-tiedostojen lisäksi myös valmiin PDF\-/tiedoston. Näissä
esimerkeissä käsitellään lähdetiedostoa nimeltä teksti.tex, mutta jos
lähdetiedostoa ei anna komennolle lainkaan, käännetään kaikki nykyisessä
hakemistossa olevat tex-päätteiset tiedostot.

\input{esim-latexmkrc.tex}

Latexmk-ohjelmalle voi tehdä asetustiedoston, johon voi kirjoittaa omaan
käyttöön sopivat asetukset. Asetustiedosto on käyttäjän kotihakemistossa
nimellä ''.latexmkrc''. Esimerkki \ref{esim:latexmkrc} näytää, mitä se
voisi ehkä sisältää.
...
esimerkissä oletus\-asetuksiin on lisätty ''non\-stop\-mode'', joka
...
(log, aux, out ym.) on \koodi{latexmk}\-/ohjelmalla jo tiedossa, mutta
rivin 4 asetuksella mukaan voi lisätä muitakin.
