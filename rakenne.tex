...
käsittelyyn, mutta avuksi on tehty monipuolisia makropaketteja. Niiden
...
Paketti \paketti{graphicx}\avctan{graphicx} sisältää toiminnot
kuvatiedostojen käsittelyyn sekä tekstin tai muun sisällön kääntelyyn ja
skaalaamisen. Kuvatiedostojen peruskäyttö sujuu komennolla
\komento{includegraphics} seuraavasti:
...
\leijutlk{
  ...
  \begin{tabularx}{\linewidth}{lL}
    ...
    \rivi{trim}{reunaviivojen siirto: vasen, ala, oikea, ylä}
    \rivi{clip}{leikkaaminen päälle}
    \bottomrule
  \end{tabularx}
}{
...
mutta kun mukaan ottaa valitsimen \koodi{clip}, kuvaa myös leikataan
reunojen ulkopuolelta. Seuraavassa esimerkissä kuvaa leikataan
...
Paketti \paketti{tikz}\avctan{tikz} tuo Latexiin monipuolisen apukielen,
jolla kirjoittaja voi piirtää vektorigrafiikkaa. Paketin taustalla
toimii toinen paketti ja grafiikkajärjestelmä \paketti{pgf} eli
...
\ymparisto{array}, joka toimii pitkälti samalla tavalla kuin tekstitilan
taulukotkin (luku \ref{luku/taulukot}). Pelkästään matematiikkatilassa
toimiva \ymparisto{array}\-/ ympäristö mahdollistaa useiden kaavarivien
latomisen ja kohdistamisen vaakasuunnassa tietylle kohdalle. Esimerkiksi
yhtälöt on mielekästä sijoittaa samalle tasalle yhtäsuuruusmerkin
kohdalta.
...
\paketti{amsmath}.\avctan{amsmath} Esimerkiksi \ymparisto{align}\-/
ympäristö pystyy tasaamaan allekkaiset kaavat tietystä kohdasta, eikä
taulukon sarakkeita tarvitse erikseen määritellä. Tasauskohta ilmaistaan
lähdedokumentissa \koodi{\&}\-/ merkillä normaalien taulukoiden tavoin.
Edellä olleen yhtälöesimerkin voi toteuttaa yksinkertaisemmin:
...
\subsection{Syntaksin perusasiat}
...
paketti \paketti{icomma}.\avctan{icomma} Paketti määrittelee
matematiikkatilan pilkun toimimaan jokseenkin älykkäästi: jos
lähdetiedostossa on pilkun jälkeen väli, pilkku ladotaan välimerkkinä;
jos väliä ei ole, pilkku katsotaan desimaalierottimeksi.

Plus- ($+$), jako- ($:$) ja yhtäsuuruusmerkki ($=$) sekä pienempi kuin
ja suurempi kuin \=/merkit ($<\,>$) kirjoitetaan näppäimistöltä
...
lähdedokumenttiin sellaisenaan mutta myös komennolla \komento{cdot}
($\cdot$) tai \komento{times} ($\times$).
...
sulkeita. Ne tehdään komentojen \komento{left} (vasen) ja
\komento{right} (oikea) avulla. Komentojen jälkeen kirjoitetaan haluttu
suljemerkki, esimerkiksi \komento{left}\komentojatko{(} ja
\komento{right}\komentojatko{)}.

\komentoi{left}
\komentoi{right}
\komentoi{frac}
\begin{koodilohkosis}
\[ 1 + \left( \frac{1}{x \left( x-1 \right)} \right) \]
\end{koodilohkosis}
\[ 1 + \left( \frac{1}{x \left( x-1 \right)} \right) \]

\noindent
Komentoja \komento{left} ja \komento{right} täytyy käyttää pareittain,
...
(\komento{left}\komentojatko{\keno\{}) mutta oikealle ei mitään
(\komento{right}\komentojatko{.}).
...
\komentoi{infty}
\begin{koodilohkosis}
\[ x^{n-1} \hfill x_i \hfill \sum_{i=1}^n \hfill
  \int_0^\infty \hfill \lim_{n \to \infty} \]
\end{koodilohkosis}
\[ x^{n-1} \hfill x_i \hfill \sum_{i=1}^n \hfill
  \int_0^\infty \hfill \lim_{n \to \infty} \]
...
$'$\=/merkki tehdään yleisheittomerkin avulla. Unicodessa se on merkki
\uctunnus{u+2032 prime}, ja matematiikassa se voi tarkoittaa esimerkiksi
funktion derivaattafunktiota. Seuraavassa esimerkissä on mukana myös
\komento{quad}\-/ komento, joka latoo pienen vaakasuuntaisen välin.
...
\noindent
Murtoluvuille ja murtoviivan latomiseen on komento \komento{frac}, jolle
annetaan kaksi argumenttia: osoittaja ja nimittäjä. Neliö- ja muut
juuret tehdään \komento{sqrt}\-/ komennolla, jolle annetaan ainakin yksi
argumentti. Komennolle voi antaa hakasulkeissa toisenkin argumentin,
joka ilmaisee juuriluvun. Seuraavassa on esimerkki murtoluvun,
neliöjuuren ja kuutiojuuren toteuttamisesta:
...
\subsection{Erikoismerkkejä ja komentoja}
...
Lisäksi on olemassa erityisiä komentoja kirjainten ja symbolien
kirjoittamiseen. Taulukkoon \ref{tlk/matem-kreikk} on koottu komennot,
joilla saa ladottua kreikkalaiset kirjaimet, ja taulukossa
\ref{tlk/matem-tarkkeet} ovat matematiikkatilan tarkekomennot.

\leijutlk{
  \providecommand{\rivi}{}
  \renewcommand{\rivi}[2]{$#1$ & \komento{#2}}
...
\leijutlk{
  \providecommand{\rivi}{}
  \renewcommand{\rivi}[4]{$#1$ & \komento{#2}\komentoarg{#3} & #4}
  ...
  \begin{tabular}{*{2}{lll}}
    ...
    \cmidrule(r){1-3}
    \cmidrule(l){4-6}
    \rivi{\grave a}{grave}{a}{gravis}
    & \rivi{\ddot a}{ddot}{a}{treema} \\
    \rivi{\acute a}{acute}{a}{akuutti}
    & \rivi{\check a}{check}{a}{hattu} \\
    \rivi{\hat a}{hat}{a}{sirkumfleksi}
    & \rivi{\breve a}{breve}{a}{lyhyysmerkki} \\
    \rivi{\widehat{aa}}{widehat}{aa}{leveä sirkumfleksi}
    & \rivi{\bar a}{bar}{a}{pituusmerkki} \\
    \rivi{\tilde a}{tilde}{a}{tilde}
    & \rivi{\dot a}{dot}{a}{yläpiste} \\
    \rivi{\widetilde{aa}}{widetilde}{aa}{leveä tilde} \\
    \bottomrule
  \end{tabular}
}{
  \caption{Tarkkeiden latominen matematiikkatilassa}
  \label{tlk/matem-tarkkeet}
}

% \[ \leq \quad \not\leq \quad \not= \quad \neq \]

% \overline \underline \underbrace \overleftarrow
%
% matriisi
% Vektori: \vec \overrightarrow
