...
''tähdettömällä'' versiolla \komentom{titleformat}. Se voi olla tarpeen
esimerkiksi \komento{chapter}\-/ tasoisissa otsikoissa, joissa halutaan
ehkä muotoilla päälukujen aloitussivua monipuolisesti. Tämän
\komento{titleformat}\-/ komentoversion monista argumenteista ja
ominaisuuksista kannattaa lukea \paketti{titlesec}\-/ paketin
ohjekirjasta.
...
komennon sisään, sen toiseen argumenttiin. Komento \komento{addto} lisää
ne suomen kielen asetuksiin (\komento{captionsfinnish}), niin että ne
tulevat voimaan samalla kun suomen kieliasetuksetkin. \komento{addto}
kuuluu \paketti{polyglossia}\-/\ ja \paketti{babel}\-/ paketteihin,
...
\subsection{Esittely, sisältö, liitteet}
...
dokumentissa. Näiden komentojen käyttö on vapaaehtoista, mutta ne voivat
olla joskus käyttökelpoisia.
...
suoraan asettamalla laskuriin \laskuri{secnumdepth} sopiva arvo.
...
näkyä. Sivutyylejä sekä ylä- ja alatunnisteita käsitellään tarkemmin
luvussa \ref{luku:yla_ala_tunnisteet}.
...
sisällysluettelon yhteydessä luvussa \ref{luku:sisallysluettelo}.
...
Dokumenttiluokassa \luokka{book} sisällysluettelon alkusivulla
suoritetaan automaattisesti komento \komento{thispagestyle} ja tyyliksi
asetetaan \koodi{plain}. Niinpä sisällysluettelon ensimmäisellä sivulla
sivunumerot näkyvät väistämättä. Monisivuisen sisällysluettelon
seuraavilla sivuilla sen sijaan noudatetaan aiemmin
\komento{pagestyle}\-/ komennolla asetettua sivutyyliä.

On kuitenkin pari erilaista tapaa pakottaa myös sisällysluettelon
ensimmäinen sivu \koodi{empty}\-/ tyyliseksi. Sen voi tehdä seuraavan
esimerkin komennolla, joka pitää sijoittaa lähdedokumentin alkuun.
Komento lisää ylimääräisen \komento{thispagestyle}\-/ komennon
sisällysluetteloa varten tehtävään väliaikaiseen \koodi{toc}\-/
päätteiseen tiedostoon. Mukana on myös \komento{protect}\-/ komento,
joka estää sen jälkeen tulevan komennon suorittamisen vielä tässä
vaiheessa.

\komentoi{addtocontents}
\komentoi{protect}
...
Vaihtoehtoinen tapa on määritellä väliaikaisesti
\komento{thispagestyle}\-/ komento siten, ettei se tee mitään.
Väliaikainen komennon määrittely saadaan aikaan aaltosulkeilla (luku
\ref{luku:aaltosulkeet}), joiden sisään komennon uudelleen määrittely ja
sisällysluettelon latomiskomento sijoitetaan.
...
% Laskuri \laskurim{tocdepth} määrittelee, mihin tasoon saakka
% sisällysluettelossa näytetään otsikoita. Esimerkiksi romaanin
% (dokumenttiluokka \luokka{book}) sisällysluettelossa voisi näyttää
% ainoastaan osat (\komento{part}) ja pääluvut (\komento{chapter})
% asettamalla laskuri seuraavasti:
...
\komento{parencite}\-/ komentoon asetuksen, että kahden viimeisen
