...
\begin{esimerkki*}
  ...
  \laskurii{enumi}
\begin{koodilohko}
  \begin{list}{\alph{enumi})}{       % kirjaimia: a) b) c) ...
...
\ref{luku:luetelma_perus}). Laskurin arvo ladotaan kirjaimina
(\komento{alph}). Laskurin \laskuri{enumi} sijasta voi käyttää mitä
tahansa omaakin laskuria, kunhan luo sen etukäteen komennolla
\komento{newcounter} (luku \ref{luku:laskurit}). Esimerkissä asetetaan
myös eräitä luetelman sisäisiä mittoja, joista on lisätietoa kuvassa
\ref{kuva:list-mitat}.
...
ympäristön ensimmäisenä argumenttina (''\koodi{merk\-ki}'') annetun
tekstin. Kirjoittaja voi määritellä \komento{makelabel}\-/ komennon
uudelleen sellaiseksi kuin haluaa. Se on tarpeen esimerkiksi silloin,
kun täytyy latoa kaikki luetelmamerkit tietynlaisella fontilla.
Varsinkin määritelmäluetelmissa (luku \ref{luku:luetelma_perus}), joissa
luetellaan käsitteitä ja niiden määritelmä, voidaan haluta, että kaikki
käsitteet ladotaan eri fontilla kuin muu teksti. Esimerkki
\ref{esim:list_makelabel} selventää tätä asiaa.

\input{kuva-list-mitat.tex}

Erilaisia \ymparisto{list}\-/ ympäristön mittoja on olemassa useita, ja
ne on koottu kuvaan \ref{kuva:list-mitat}. Seuraavassa on lisätietoa
mittojen merkityksestä.

\begin{maaritelma}{\mitta{#1}}
\item [parskip] Pystysuuntainen mitta ja väli, joka ei liity pelkästään
  \ymparisto{list}\-/ ympäristöön, vaan se on yleinen tekstikappaleiden
  välinen väli (luku \ref{luku:pystysuuntaiset_valit}). Mitta on
  vaikuttaa kuitenkin myös \ymparisto{list}\-/ ympäristöön, ja sen voi
  määritellä väliaikaisesti uudelleen ympäristön yhteydessä.
...
\item [leftmargin] Luetelman kappaleiden vasen sisennys. Tämä tarkoittaa
  luetelmamerkistä seuraavien rivien sisennystä.
\item [rightmargin] Luetelman kappaleiden oikea sisennys. Oletusarvo on
  nolla.
\item [labelwidth] Luetelmamerkin leveys. Merkki ladotaan tämän alueen
  oikeaan reunaan. Jos luetelmamerkki eli \komento{makelabel}\-/
  komennon latoma teksti on leveämpi kuin tämä mitta, luetelman tekstin
  ensimmäinen rivi siirtyy oikealle siten, että luetelmamerkki mahtuu.

  Luetelmamerkin alueen vasen reuna sijaitsee kohdassa, jonka voi laskea
  seuraavasta kaavasta: \mitta{leftmargin} + \mitta{itemindent} −
  \mitta{labelsep} − \mitta{labelwidth}. Mikäli haluaa, että
  luetelmamerkin vasen reuna on sisennyksen nollakohdassa, täytyy edellä
  mainitut mitat asettaa siten, että mittojen yhteistulos on nolla.
  Esimerkissä \ref{esim:list_makelabel} näin on juuri tehty.
