...
merkinnän sisällysluetteloon (\koodil{toc}, engl.
...
otsikkorivit tai yhteenvetorivit.
...
leipätekstin sekaan tiettyyn kohtaan. Ne ''leijuvat'' muun tekstin
ulkopuolella, ja Latex huolehtii niiden sijoittamisesta. Leijuvilla
osilla on yleensä kuvateksti, jonka alussa on sen tyyppi ja numero,
esimerkiksi ''Kuva~1'' tai ''Taulukko 4.2''. Näiden perässä on
varsinainen kuvateksti, joka kertoo kyseisen leijuvan osan sisällöstä
tai merkityksestä. Leipätekstistä voi viitata leijuviin osiin
käyttämällä ilmauksia kuten ''kuvassa~1''. Näissä hyödynnetään Latexin
ristiviittaustoimintoja (luku \ref{luku:ristiviitteet}).

Leijuvien osien keskinäinen järjestys säilyy osittain: samantyyppiset
leijuvat osat, esimerkiksi kuvat, ladotaan aina siinä järjestyksessä
kuin ne ovat lähdedokumentissa. Sen sijaan erityyppiset leijuvat osat
saattavat sekoittua: esimerkiksi jokin taulukko saatetaan latoa ennen
...
Perus Latex tarjoaa kaksi erityyppistä leijuvaa osaa, kuvat ja taulukot,
mutta \pakettim{newfloat}\-/ makropaketin avulla niitä voi tehdä lisää.
Tässä oppaassa on käytössä myös tyyppi ''Esimerkki'', jota käytetään
leijuvien Latex\-/ koodiesimerkkien toteuttamiseen. Seuraavissa
alaluvuissa käsitellään ensin perus Latexin ominaisuudet ja asiaan
liittyvät käsitteet. Sitten annetaan vinkkejä omien leijuvien osien
tekemisestä ja asettelumahdollisuuksista muiden makropakettien avulla.

\subsection{Leijuvat taulukot ja kuvat KESKEN}

Käytännössä leijuvat osat ovat ympäristöjä, joiden sijoittelusta vastaa
Latex eikä kirjoittaja. Tosin kirjoittajakin voi hieman vaikuttaa asiaan
(luku \ref{luku:leijuosat_asetukset}). Ympäristö \ymparistom{table} on
tarkoitettu taulukoille, ja ympäristö \ymparistom{figure} on kuville.
Molemmat toimivat samoin, mutta esimerkissä \ref{esim:leijuosat_table}
käytetään taulukkoa.
...
kieliasetuksilla sana ''Taulukko'', ja \ymparisto{figure}\-/
ympäristössä se on ''Kuva''. Näiden muuttamista käsitellään asetusten
yhteydessä luvussa \ref{luku:leijuosat_asetukset}.

Kuvatekstit tehdään komennolla \komentom{caption}, joka sijoitetaan
leijuvan ympäristön sisälle, kuten esimerkissä
\ref{esim:leijuosat_table} on tehty. Komennolle annetaan yksi argumentti
eli haluttu kuvateksti. Siihen ei pidä kirjoittaa leijuvan osan tyyppiä
eikä numeroa (esim. ''Taulukko~3''), sillä nämä Latex tekee
automaattisesti. \komento{caption}\-/ komennolle voi antaa myös
valinnaisen argumentin, jolla ilmaistaan kuvatekstistä lyhyempi versio:
...
ristiviitteissä. Leijuvista osista voi nimittäin latoa automaattisen
luettelon esimerkiksi dokumentin alkuun tai loppuun, ja luettelossa
käytetään kuvatekstin lyhyttä versiota, jos sellainen on annettu. Myös
ristiviittauksissa (luku \ref{luku:ristiviitteet}) voidaan käyttää
kuvatekstiä, ja silloin niihin tulee lyhempi versio.

Leijuvien osien luettelot voidaan latoa tyypeittäin: komennolla
\komentom{listoftables} syntyy taulukkoluettelo ja komennolla
\komentom{listoffigures} kuvaluettelo. Nämä muistuttavat
sisällysluetteloita (luku \ref{luku:sisallysluettelo}), eli niille tulee
automaattisesti otsikko kuten ''Taulukot'' ja ''Kuvat'' ja
automaattisesti sisältö eli kaikki \komento{caption}\-/ komennolla
ilmaistut (lyhyet) kuvatekstit.

% table*, figure*

\subsection{Sijoittelu ja asetuksia KESKEN}
\label{luku:leijuosat_asetukset}
...
Kuvatekstissä näkyvät, leijuvan osan tyyppiä ilmaisevat sanat
''Taulukko'' tai ''Kuva'' tulevat komennoista \komento{tablename} ja
\komento{figurename}, jotka kirjoittaja voi määrittää uudelleen:
