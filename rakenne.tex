...
Komennon argumentti \koodil{nimi} on kirjoittajan itse valitsema nimi,
ja se voi liittyä esimerkiksi värin käyttötarkoitukseen (''nuoli'') tai
värin yleiseen nimeen (''keltainen''). Argumentti \koodil{väri\-malli}
tarkoittaa värin ilmaisemisen tapaa. Niitä on useita erilaisia, mutta
yleisimmät on koottu taulukkoon \ref{tlk:varimalleja}. Eri malleilla on
...
\koodil{para\-metrit} ja ne erotetaan toisistaan pilkulla. Taulukon
\ref{tlk:varimalleja} Esimerkki\-/sarakkeessa on parametrien antamisesta
...
\koodil{toc} (\englantik{table of contents}), joka on
...
\koodil{section} eli vastaa muita \komento{section}\-/ komennolla
...
Edellisen esimerkin argumentti \koodil{merk\-ki} sisältää tekstiä tai
...
\ymparisto{list}\-/ ympäristön toinen argumentti \koodil{ase\-tuk\-set}
...
määritelmäluetelmissa (luku \ref{luku:luetelma_perus}), joissa
...
\ref{esim:list_makelabel} selventää tätä asiaa.
...
Esimerkki \ref{esim:list_oma_numeroitu} tekee numeroidun ympäristön
...
\mitta{parindent} (luku \ref{luku:ensimmaisen_rivin_sisennys}). Jos
...
Ympäristön valinnainen argumentti \koodil{sijainti} määrittelee
taulukkoympäristön sijainnin pystysuunnassa, jos se on samassa
kappaleessa muun tekstin kanssa. Oletusarvo on \koodil{c}
(\englanti{center}), joka tarkoittaa, että pystysuunnassa taulukon
peruslinja on sen keskellä. Muut vaihtoehdot ovat \koodil{t}
(\englanti{top}), eli taulukon peruslinja on yläreunassa, ja \koodil{b}
...
\leijutlk{
  ...
  \begin{tabular}{ll}
    ...
    \rivi{p\{m\}}{sarake, jonka leveys on mitta \koodil{m}}
    \midrule
    \rivi{|}{sarakkeiden välissä pystyviiva}
    \rivi{@\{…\}}{sarakkeiden välissä olevaa tekstiä tai komentoja}
    \rivi{*\{n\}\{s\}}{toistetaan \koodil{n} kertaa sarakkeet \koodil{s}}
    \bottomrule
  \end{tabular}
}{
  \caption{Taulukoiden saraketyyppien ja sarakkeiden välien määrittely}
  \label{tlk:taulukko_sarakemerkit}
}

Ensimmäinen pakollinen argumentti \koodil{sarakkeet} määrittelee
...
\koodil{lcrp} määrittelevät, millaisesta sarakkeesta on kyse. Syntyvässä
taulukossa tulee olemaan niin monta saraketta kuin näitä on ympäristön
\koodil{sarakkeet}\-/ argumentissa. Kolme ensin mainittua luovat
sarakkeen, jonka leveys määräytyy sarakkeen leveimmän solun perusteella.
Neljäs eli saraketyyppi \koodil{p} tarvitsee aaltosulkeissa argumentiksi
...
Merkit \koodil{|} ja \koodil{@} voi sijoittaa edellä mainittujen
...
käsitellään myöhemmin. Merkki \koodil{@} tarvitsee argumentin
aaltosulkeissa. Se voi olla mitä hyvänsä tekstiä tai komentoja, jotka
halutaan suorittaa joka rivillä sarakkeiden välissä.

Sarakemerkki \koodil{*} on vain keino toistaa tiettyjä sarakemäärityksiä
...
Esimerkiksi \koodil{*\{3\}\{rl|\}} tarkoittaa samaa kuin
\koodil{rl|rl|rl|}.
...
Esimerkissä \ref{esim:taulukko_desimaalipilkku} on taulukoitu lukuja,
...
soluun. Komennon ensimmäinen argumentti \koodil{n} on luku ja ilmaisee,
kuinka monen sarakkeen alueeseen vaikutetaan. Toinen argumentti
\koodil{sarak\-keet} sisältää uuden sarakemäärittelyn tälle alueelle, ja
se tehdään taulukon \ref{tlk:taulukko_sarakemerkit} merkkien avulla.
...
Saraketyyppi~\koodil{p} (taulukko \ref{tlk:taulukko_sarakemerkit}) on
...
Ensimmäinen argumentti \koodil{n} on luku, joka ilmaisee, kuinka monen
rivin alueelle solu levitetään. Toinen argumentti \koodil{leveys} on
...
Edellisen esimerkin valinnainen argumentti \koodil{sijainti} on kirjain,
jolla ilmaistaan solun sisällön pystysuuntainen asemointi ylös, keskelle
tai alas: \koodil{t} (\englanti{top}), \koodil{c} (\englanti{center},
oletus) tai \koodil{b} (\englanti{bottom}). Valinnainen argumentti
\koodil{pysty} on mitta, jolla hienosäädetään solun sisällön sijaintia
...
saraketyypin:~\koodil{X} (vrt. taulukko
\ref{tlk:taulukko_sarakemerkit}). Tämä saraketyyppi toimii kuten
tyyppi~\koodil{p}, mutta sille ei anneta argumentiksi mittaa, vaan sen
...
Saraketyypissä \koodil{X} teksti tasataan oletuksena solujen
...
\komento{raggedright} vaikutuksesta (luku \ref{luku:kappaleen_tasaus}).
...
\koodil{LCR}, jotka toimisivat lähes samoin kuin perus Latexin
saraketyypit \koodil{lcr} (taulukko \ref{tlk:taulukko_sarakemerkit})
...
taulukon ensimmäiseen, otsikoita ilmaisevaan saraketyyppiin \koodil{O}
...
paketin\avctan{xcolor} (luku \ref{luku:korostus_varit}) komentojen
...
\subsection{Sarkaimet KESKEN}
\label{luku:taulukot_tabbing}

...

% tabbing-ympäristö, voi ylittää sivurajan.

\section{Leijuvat osat KESKEN}
\label{luku:leijuosat}

% Muutetaan newfloat-paketin tilalle floatrow
...
(luku \ref{luku:leijuosat_latex}), mutta \pakettim{newfloat}\-/
makropaketin avulla niitä voi tehdä muitakin (luku
\ref{luku:leijuosat_omat}). Esimerkiksi tässä oppaassa on käytössä myös
tyyppi ''Esimerkki'', jota käytetään leijuvien Latex\-/ koodiesimerkkien
toteuttamiseen.
...
asiaan, ja tätä käsitellään luvussa \ref{luku:leijuosat_sijoittelu}.
Ympäristö \ymparistom{table} on tarkoitettu taulukoille, ja ympäristö
\ymparistom{figure} on kuville. Esimerkistä \ref{esim:leijuosat_perus}
...
\ref{esim:leijuosat_perus} on tehty. Komennolle annetaan yksi argumentti
...
\ref{luku:leijuosat_luettelot}) ja ristiviitteissä. Kuvatekstin eli
...
\subsection{Muut leijuvat osat KESKEN}
\label{luku:leijuosat_omat}

% Muutetaan newfloat-paketin tilalle floatrow

Omia, itse nimettyjä leijuvia osia tehdään \pakettim{newfloat}\-/
paketin\avctan{newfloat} avustuksella. Sen tärkein komento on
\komentom{DeclareFloatingEnvironment}, jonka argumentit ovat
seuraavanlaiset:

\komentoi{DeclareFloatingEnvironment}
\begin{koodilohkosis}
\DeclareFloatingEnvironment[valitsimet]{tyyppi}
\end{koodilohkosis}

Komennon pakollinen argumentti \koodil{tyyppi} on leijuvan osan tyyppi
...
\ymparistox{esimerkki*}, joiden kuvateksteissä on sana ''Esimerkki'' eli
sama sana mutta isolla alkukirjaimella. Automaattisesti syntyy myös uusi
\komentox{listof}\-/ alkuinen komento
\komentox{list\-of\-esi\-merk\-kis} (tai \koodi{-es}), jota voi käyttää
luettelon (luku \ref{luku:leijuosat_luettelot}) latomiseen
tämäntyyppisistä leijuvista osista.
...
\leijutlk{
  ...
  \begin{tabular}{ll}
    ...
    \rivi{listname}{luettelon otsikko}
    \rivi{placement}{sijoitteluasetukset}
    \rivi{fileext}{väliaikaistiedoston pääte}
    \rivi{within}{ylemmäntasoinen laskuri (tai \koodi{none})}
    \rivi{chapterlistsgaps}{luettelossa väli päälukujen välissä
    (\koodi{on} tai \koodi{off})}
    \bottomrule
  \end{tabular}
}{
  \caption{\komento{DeclareFloatingEnvironment}\-/ komennon valitsimia,
    joilla vaikutetaan leijuvan osan ominaisuuksiin}
  \label{tlk:declarefloat}
}
...
\DeclareFloatingEnvironment[
  name=Esimerkki,
  listname={Luettelo esimerkeistä},
  placement=tbp,
  fileext=loesim,
  within=chapter,
...
\caption{\komento{DeclareFloatingEnvironment}\-/ komennon käyttö}
\label{esim:declarefloat}
\end{esimerkki*}

Argumentin \koodil{valitsimet} avulla voi vaikuttaa tarkemmin leijuvan
osan ominaisuuksiin. Mahdolliset valitsimet on koottu taulukkoon
\ref{tlk:declarefloat}. Eri valitsimet erotetaan toisistaan pilkulla,
eli komentoa käytetään esimerkin \ref{esim:declarefloat} tavoin.

Valitsimen \koodil{name} arvo näkyy kuvateksteissä leijuvan osan
tyyppinä ja valitsimen \koodil{listname} arvo on puolestaan otsikko,
joka ladotaan tämäntyyppisten leijuvien osien luettelon alkuun (luku
\ref{luku:leijuosat_luettelot}). Valitsimella \koodil{placement} tehdään
sijoitteluasetuksia, joita käsitellään luvussa
\ref{luku:leijuosat_sijoittelu}. Käytännössä ne ovat tiettyjä kirjaimia,
joilla vaikutetaan leijuvan osan sijoitteluun. Oletusarvo on
\koodil{tbp}.

Valitsin \koodil{file\-ext} on tiedoston pääte. Leijuvien osien
...
tiedostopäätettä, joka alkaa kirjaimilla \koodil{lo} (\englantik{list
  of}). Latexin omissa leijuvissa osissa päätteet ovat \koodil{lot}
(\englantik{list of tables}) ja \koodil{lof} (\englantik{list of
  figures}). Oletuksena omille leijuville osille tulee päätteeksi
\koodil{lo}, jonka perässä on osan tyyppi.

Valitsimen \koodil{within} avulla vaikutetaan leijuvien osien
numeroinnin riippuvuuteen muista laskureista. Oletusarvo on
\koodil{none}, joka tarkoittaa, että leijuvan osan laskuri ei ole
riippuvainen muista laskureista eli osat numeroidaan dokumentin alusta
saakka yhdellä luvulla. Arvoksi voi antaa myös Latexin laskurin nimen:
mielekkäitä arvoja ovat esimerkiksi \laskuri{chapter} ja
...
arvo kasvaa. Lisäksi kuvateksteissä näkyy kaksiosainen numerointi (4.2,
4.3 jne.), jossa ensimmäinen luku on ylemmäntasoisen laskurin arvo --
...
luvussa \ref{luku:hierarkkiset_laskurit}.

Jos valitsimen \koodil{chapterlistsgaps} arvo on \koodil{on},
luettelossa eri päälukuihin kuuluvat leijuvat osat erotetaan toisistaan
pienellä pystysuuntaisella välillä. Tämä kytketään pois antamalla
arvoksi \koodil{off}. Asetuksella on merkitystä vain dokumenttiluokissa,
joissa on \komento{chapter}\-/ otsikot. Esimerkiksi \luokka{article}\-/
luokassa ei ole.

Paketti \paketti{newfloat} mahdollistaa myös aiemmin määriteltyjen
leijuvien osien muokkaamisen jälkikäteen. Se tehdään komennolla
\komentom{SetupFloatingEnvironment}, jolle annetaan argumentiksi
leijuvan osan tyyppi eli ympäristön nimi (\ymparisto{table},
\ymparisto{figure} tms.) ja muutettavat asetukset (taulukko
\ref{tlk:declarefloat}).

\komentoi{SetupFloatingEnvironment}
\begin{koodilohkosis}
\SetupFloatingEnvironment{tyyppi}{valitsimet}
\end{koodilohkosis}

\subsection{Leijuvien osien luettelot}
\label{luku:leijuosat_luettelot}
...
\komentom{listoffigures} kuvaluettelo. Omien leijuvien osien luettelo
tehdään vastaavalla \komentox{listof}\-/ alkuisella komennolla (luku
\ref{luku:leijuosat_omat}).

Leijuvien osien luettelot muistuttavat sisällysluetteloita (luku
\ref{luku:sisallysluettelo}), eli niille tulee automaattisesti otsikko
...
\end{koodilohko}
  \caption{Leijuvien osien ulkoasua muokataan \komento{titlecontents}\-/
    komennolla, joka on peräisin \paketti{titletoc}\-/ paketista}
  \label{esim:leijuosat_titlecontents}
\end{esimerkki*}
...
Esimerkistä \ref{esim:leijuosat_titlecontents} selvinnee perusajatus, ja
lisätietoa on sisällysluetteloita käsittelevässä luvussa
\ref{luku:sisallysluettelo} sekä tietysti paketin ohjekirjassa.
...
sisällysluetteloihinkin, komennolla \komento{addcontentsline}. Komennon
ensimmäinen argumentti on luettelon tyyppiä vastaavan tiedoston pääte.
Tyyppi \koodil{lot} (\englantik{list of tables}) on taulukoille ja
tyyppi \koodil{lof} (\englantik{list of figures}) on kuville. Omilla
leijuvilla osilla (luku \ref{luku:leijuosat_omat}) se on jotakin muuta
...
\subsection{Sijoittelu}
\label{luku:leijuosat_sijoittelu}
...
\leijutlk{
  ...
  \begin{tabular}{cl}
    ...
    \rivi{H}{ehdottomasti tähän kohtaan (\paketti{floatrow}, \paketti{float})}
    \rivi{!}{ei huomioida tiettyjä sijoittelurajoituksia}
    \bottomrule
  \end{tabular}
}{
  \caption{Leijuvien osien sijoitteluun voi vaikuttaa ympäristön
    valinnaisen argumentin avulla. Valitsin \koodil{H} on
    \paketti{floatrow}\-/\ ja \paketti{float}\-/ pakettien ominaisuus}
  \label{tlk:leijuosat_sijoittelukirjaimet}
}

Valinnainen argumentti \koodil{sijainti} on tietty merkki tai merkkien
yhdistelmä, jolla vaikutetaan leijuvan osan sijoitteluun. Taulukossa
\ref{tlk:leijuosat_sijoittelukirjaimet} ovat mahdolliset vaihtoehdot.
Oletusarvo on \koodil{tbp}, eli sen mukaan leijuva osa pyritään
...
Vaihtoehto \koodil{h} pyrkii latomaan leijuvan osan siihen kohtaan kuin
...
makropaketin \pakettim{floatrow}\avctan{floatrow} (tai yksinkertaisemman
\paketti{float}\-/ paketin\avctan{float}) voi käyttää myös myös
valitsinta \koodil{H}. Se tarkoittaa ehdotonta vaatimusta, että juuri
tähän kohtaan leijuva osa pitää sijoittaa. Tätä valitsinta ei voi
yhdistää muiden valitsimien kanssa.

Valitsinta \koodil{!} käytetään yhdessä edellä mainittujen valitsimien
\koodil{tbp} kanssa. Se poistaa tietyt sijoitteluun vaikuttavat säännöt
...
ympäristön valitsinta \koodil{!} (taulukko
\ref{tlk:leijuosat_sijoittelukirjaimet}).
...
\item [bottomnumber] Sivun alaosaan sijoitettavien leijuvien osien
  ...
  ei sijoiteta koskaan sivun alaosaan.
...
\item [floatsep] Samalla sivulla kahden peräkkäisen leijuvan osan
  pystysuuntainen väli. Oletusarvo on venyvä mitta \koodil{12pt plus 2pt
    minus 2pt}. Kahden palstan tilassa käytetään mittaa
  \mitta{dblfloatsep}.
\item [textfloatsep] Leijuvan osan ja leipätekstin välinen
  pystysuuntainen väli. Oletusarvo on venyvä mitta \koodil{20pt plus 2pt
...
\item [intextsep] Pystysuuntainen väli leipätekstin ja leijuvan osan
  ...
  koskee sijoitteluasetuksia \koodil{h} ja \koodil{H} (taulukko
  \ref{tlk:leijuosat_sijoittelukirjaimet}). Oletusarvo vaihtelee
  dokumenttiluokan fonttikokoasetusten (luku
  \ref{luku:perusdokumenttiluokat_asetukset}) perusteella. Jos
  dokumenttiluokan valitsin on \koodil{10pt} (oletus) tai \koodil{11pt},
  oletusmitta on \koodil{12pt plus 2pt minus 2pt}. Dokumenttiluokan
  valitsimella \koodil{12pt} oletus on \koodil{14pt plus 4pt minus 4pt}.
\end{maaritelma}
...
\komento{renewcommand} (luku \ref{luku:komennot_maarittely}). Näiden
vaikutuksen voi poistaa kokonaan leijuvan ympäristön valitsimella
\koodil{!} (taulukko \ref{tlk:leijuosat_sijoittelukirjaimet}).
...
\item [topfraction] Sivun yläosaan sijoitettavien leijuvien osien viemä
  ...
  on~\koodil{0.7}, mikä tarkoittaa, että leijuvat osat voivat viedä
...
\item [bottomfraction] Sivun alaosaan sijoitettavien leijuvien osien
  ...
  on~\koodil{0.3} (30\,\%). Kahden palstan tilassa leijuvia osia ei
  koskaan sijoiteta sivun alaosaan.
\item [textfraction] Leipätekstin vähimmäistila sivulla, kun samalla
  sivulla on myös leijuvia osia. Oletusarvo on \koodil{0.2} (20\,\%).
\item [floatpagefraction] Omalle sivulleen sijoitettavien leijuvien
  ...
  lainkaan. Oletusarvo on~\koodil{0.5} (50\,\%). Kahden palstan tilassa
...
\ref{luku:otsikot}) tekevät myös saman automaattisesti joissakin
dokumenttiluokissa.
...
\koodil{section}, asetetaan tällainen raja automaattisesti
...
\subsection{Typografia ja muita asetuksia}
