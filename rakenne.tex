...
voidaan esimerkiksi ladata ja latoa kuvatiedostoja sekä piirtää verkkoja
ja vektorigrafiikkakuvia Latex\-/ komentojen avulla.
...
\subsection{Vektorigrafiikka KESKEN}

Paketti \paketti{tikz}\avctan{tikz} tuo Latexiin monipuolisen apukielen,
käytännössä Latexin komentoja, joilla kirjoittaja voi piirtää
vektorigrafiikkaa. Paketin taustalla toimii toinen paketti ja
grafiikkajärjestelmä \paketti{pgf} eli \englantik{portable graphics
  format}, mutta kirjoittajan ei yleensä tarvitse välittää taustalla
olevasta matalan tason tekniikasta.
...
perusasetuksia. Tässä asetetaan x- ja y\=/ koordinaattien suuntaiset
yksikkömitat 1\,mm:n pituiseksi. Oletusyksikkö on 10\,mm.
Piirustusalustan origo eli koordinaatti (0,~0) sijaitsee vasemmassa
alanurkassa, ja koordinaatisto kasvaa ylös ja oikealle.

Komennolla \komento{draw} piirretään viivakuvio ilmaisemalla sulkeissa
välipisteiden koordinaatit. Lopussa oleva \koodi{cycle} piirtää viivan
takaisin saman \komento{draw}\-/ komennon alkupisteeseen. Tätä
kokonaisuutta kutsutaan poluksi, ja sen lopussa täytyy olla puolipiste
(\koodi{;}). Esimerkissä on toinenkin polku eli toinen \komento{draw}\-/
komento piirtää pystyviivan suuren kolmion keskelle ja jakaa sen
kahdeksi pienemmäksi kolmioksi.

Koordinaatit voi ilmaista suhteessa polun edelliseen pisteeseen
lisäämällä koordinaatin eteen kaksi plusmerkkiä. Seuraavan esimerkin
neljä ensimmäistä pistettä ilmaistaan normaalisti x- ja y\=/
koordinaateilla, mutta viimeinen piste ilmaistaan kulman (25°) ja
pituuden (8) avulla.
...
peruskuvioille. Seuraavassa piirretään suorakulmio, ympyrä ja ellipsi:
...
\noindent
Kuten edellinen esimerkki osoittaa, piirtokomennoille voi antaa
hakasulkeissa lisätietoja. Mahdollisuuksia on valtavan paljon,
esimerkiksi värit, viivojen paksuus, nuolenkärjet ja kulmien pyöristys.
Esimerkki \ref{esim/tikz-asetuksia} havainnollistaa tavallisimpia
valinnaisia argumentteja. Värivalitsimien \koodi{color} ja \koodi{fill}
arvoksi voi antaa mitä hyvänsä nimettyjä värejä, joita käsitellään
tarkemmin luvussa \ref{luku/korostus-värit}.
...
\begin{koodilohko}
\begin{tikzpicture}[x=1mm, y=1mm,
  nuoli/.style={line width=1bp, color=red}]

  % Nuolia
  \draw [nuoli, ->] (0,0) -- (10,5);
  \draw [nuoli, <<-, color=blue] (10,0) -- (20,5);
  \draw [nuoli, <->] (20,0) -- (30,5);

  % Täytetty nelikulmio, viivan paksuus ja kulmien pyöristys
  \draw [color=black, fill=yellow, line width=1.5bp, rounded
...
\subsection{Solmut, nuolet ja verkot KESKEN}

...

% graphviz

\subsection{Tekstipalstaan upottaminen KESKEN}

...

% wrapfig-paketti

\section{Diaesitykset KESKEN}
