...
\koodil{toc} (engl. \englanti{\textit{table of contents}}), joka on
...
numeron (1.2, 1.3 tms.) tietynlevyiseen tilaan. Komento
...
Leijuvat osat (engl. \englanti{\textit{float}}) ovat sellaisia
dokumentin osia -- käytännössä ympäristöjä~--, joita ei ladota normaalin
leipätekstin sekaan tiettyyn kohtaan. Ne ''leijuvat'' irrallaan
tekstistä eli ovat muun tekstin ulkopuolella. Latex huolehtii niiden
sijoittamisesta. Leijuvilla osilla on yleensä kuvateksti, jonka alussa
on sen tyyppi ja numero, esimerkiksi ''\figurename~1'' tai ''\tablename\
4.2''. Näiden perässä on varsinainen kuvateksti, joka kertoo kyseisen
leijuvan osan sisällöstä tai merkityksestä. Leipätekstistä voi viitata
leijuviin osiin käyttämällä ilmauksia kuten ''kuvassa~1''. Näissä
hyödynnetään Latexin ristiviittaustoimintoja (luku
...
järjestyskin voi hieman poiketa siitä, miten ne esiintyvät Latex\-/
...
\subsection{Leijuvat taulukot ja kuvat KESKEN}
...
lyhempää versiota käytetään mahdollisesti luettelossa ja
ristiviitteissä. Kuvatekstin eli \komento{caption}\-/ komennon jälkeen
leijuvaan ympäristöön lisätään usein yksilöllinen tunniste
ristiviittausta varten. Se tehdään \komento{label}\-/ komennolla, jota
käsitellään ristiviitteiden yhteydessä luvussa \ref{luku:ristiviitteet}.
...
\titlecontents{figure}                      % kuvaluettelo (figure)
[8mm]                                       % vasen sisennys (mitta)
{\addvspace{3bp}\rmfamily\small}            % yläpuolinen koodi
{\contentslabel{8mm}}                       % numeroitu kohta
{}                                          % numeroimaton kohta
...
Tyyppi \koodil{lot} (engl. \englanti{\textit{list of tables}}) on
taulukoille ja tyyppi \koodil{lof} (\englanti{\textit{list of figures}})
on kuville.
...
% newfloat-paketti, omien leijuosien luettelot
% floatrow-paketti
% caption-paketti
